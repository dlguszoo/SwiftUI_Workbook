...
class ReceiptsViewModel {
    var selectedSegment: ReceiptSegment = .first {
        didSet { // Property Observer - didSet: 새로운 값이 저장된 직후에 호출. 이전 프로퍼티 값이 oldValue로 제공
    ...
    private func parseWithoutRegax(from text: String) -> ReceiptsModel {
        ...
        while i < lines.count {
            ...
            if isMenuSection,
               ...
               !trimmed.contains("데움") {
                ...
                if let cleanName = trimmed.components(separatedBy: ")").last?.trimmingCharacters(in: .whitespaces) {
        ...
        return ReceiptsModel(orderer: orderer, store: store, menuItems: menuItems, totalAmount: totalAmount, orderNumber: orderNumber)
    }
}
